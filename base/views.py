...
def home(request): 
    ...
    room_message = Message.objects.filter(Q(room__topic__name__icontains = q) | Q(room__name__icontains=q) |
...
def deleteMessage(request, pk):
    ...
    if request.method == 'POST':
        ...
        user = message.user
        message.delete()
        # If the user has no more messages in this room, remove them from participants
        if not room.message_set.filter(user=user).exists():
...
def editMessage(request, pk):
    ...
    if request.user != message.user:
        return HttpResponse('You cannot edit this message')
